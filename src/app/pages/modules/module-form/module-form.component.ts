...
import { ModuleContent, ContentType } from '../../../core/models/content.model';
      ...
      <div class="page-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1>{{ (isEdit() ? 'modules.edit' : 'modules.add') | translate }}</h1>
            <p>{{ isEdit() ? 'Modul ma\'lumotlarini yangilash' : 'Yangi modul yaratish' }}</p>
          </div>
        </div>
      </div>
          ...
          <mat-tab-group class="modern-tabs">
            <mat-tab>
              ...
              <div class="tab-content">
                <form (ngSubmit)="onSave()" #moduleForm="ngForm" class="modern-form">
                  <div class="form-section">
                    <h3 class="section-title">Asosiy ma'lumotlar</h3>
                    
                    <div class="input-group">
                      <label for="title">{{ 'modules.name' | translate }} *</label>
                      <div class="input-wrapper" [class.error]="title.invalid && title.touched">
                        <mat-icon>title</mat-icon>
                        <input id="title" [(ngModel)]="form.title" name="title" 
                               placeholder="Modul nomini kiriting" required #title="ngModel">
                      </div>
                      @if (title.invalid && title.touched) {
                        <span class="error-text">Modul nomi kiritilishi shart</span>
                      }
                    </div>
                    ...
                    <div class="input-group">
                      ...
                      <div class="input-wrapper textarea-wrapper">
                        ...
                        <textarea id="description" [(ngModel)]="form.description" name="description" 
                                  rows="4" placeholder="Modul haqida qisqacha ma'lumot"></textarea>
                      </div>
                    </div>

                    <div class="form-grid">
                      <div class="input-group">
                        <label for="thumbnail">{{ 'modules.thumbnail' | translate }}</label>
                        <div class="input-wrapper">
                          <mat-icon>image</mat-icon>
                          <input id="thumbnail" [(ngModel)]="form.thumbnail" name="thumbnail" 
                    ...
                    <div class="toggle-group">
                      <div class="toggle-item">
                        <div class="toggle-info">
                          ...
                          <div>
                            <span class="toggle-label">{{ 'modules.sequential' | translate }}</span>
                            <span class="toggle-desc">Darslar tartib bilan o'tiladi</span>
                          </div>
                        </div>
                      ...
                      <div class="toggle-item">
                        <div class="toggle-info">
                          ...
                          <div>
                            ...
                            <span class="toggle-desc">Modul talabalarga ko'rinadi</span>
                          </div>
                        </div>
                  ...
                  <div class="form-actions">
                    <button type="button" class="btn btn-secondary" (click)="router.navigate(['/modules'])">
                      <mat-icon>close</mat-icon>
                      {{ 'common.cancel' | translate }}
                    </button>
                    <button type="submit" class="btn btn-primary" [disabled]="saving() || moduleForm.invalid">
                      @if (saving()) {
            ...
            @if (isEdit()) {
              <mat-tab>
                <ng-template mat-tab-label>
                  ...
                  <span>{{ 'modules.contents' | translate }}</span>
                </ng-template>
...
export class ModuleFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.moduleService.getById(id).subscribe({
        next: (mod) => {
          this.form = { title: mod.title, description: mod.description, thumbnail: mod.thumbnail, order_index: mod.order_index, is_sequential: mod.is_sequential, is_published: mod.is_published };
          this.contents.set(mod.contents || []);
  ...
  onSave(): void {
    ...
    if (this.isEdit()) {
      this.moduleService.update(this.moduleId, this.form).subscribe({
        next: () => this.router.navigate(['/modules']),
        error: () => this.saving.set(false)
      });
    } else {
      this.moduleService.create({ ...this.form, teacher: teacher?.id }).subscribe({
        next: () => this.router.navigate(['/modules']),
        error: () => this.saving.set(false)
      });
    }
  ...
  saveContent(): void {
    this.contentService.createModuleContent({ ...this.contentForm, module: this.moduleId }).subscribe({
      next: (c) => {
  ...
  deleteContent(id: string): void {
    ...
    ref.afterClosed().subscribe(result => {
      if (result) {
        this.contentService.deleteModuleContent(id).subscribe(() => {
